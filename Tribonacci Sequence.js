...
// [0, 1] instead of[1, 1] basically shifts the common Fibonacci sequence by once place, 
...
// SOLITION 
// 1- Initialize result array with the signature (first 3 numbers)
// 2- If n is 0, return an empty array
// 3- If n <= 3, return the first n elements of the signature
// 4- For n > 3, calculate the next numbers
// 5- Each new number is the sum of the last 3 numbers in the result array
// 6- Repeat step 5 until the result array has n numbers



function tribonacci(signature, n) {
    let result = signature.slice(0, 3);
    if (n === 0) return [];
    if (n <= 3) return result.slice(0, n);
    while (result.length < n) {
        let lastThree = result.slice(-3);
